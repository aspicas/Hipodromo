...
--Historia de jinete con que ejemplar a ganado y cual carrera 
...
-- Estadistica del ejemplar combinado jinete y entrenador 
SELECT EJ.EJEM_NOMBRE ,CA.CAEJ_POSICION ,J.JINE_NOMBRE,E.ENTR_NOMBRE , CAR.CARR_NOMBRE
FROM  ENTRENADOR E , JINETE J , CAEJ CA, CARRERA CAR, EJEMPLAR EJ, CABALLERIZA CAB , PUESTO PU
WHERE CA.FK_EJEM=EJ.EJEM_CODIGO  AND CA.FK_CARR=CAR.CARR_CODIGO 
AND CA.FK_JINE=J.JINE_CODIGO AND CAB.FK_ENTR=E.ENTR_CODIGO AND PU.FK_CABA=CAB.CABA_CODIGO AND EJ.FK_PUES=PU.PUES_CODIGO;
...
--Indicar cuales son los implementos mas utilizados en las carreras de mayor a menos  segun su porcentaje de uso 
...
-- el promedio de ejemplares que corrieron en las ultimas 50 carreras segun su pelaje 
