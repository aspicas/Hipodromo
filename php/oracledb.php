...
function DesplegarC6Valores($string,$value1,$value2,$value3,$value4,$value5,$value6)
{
	global $conn;
	$query = oci_parse($conn, $string);
	oci_execute($query);	
	while (($row = oci_fetch_assoc($query)) != false) {		
		echo "<option value='".$row[$value1]."'>".$row[$value2].", ".$row[$value3].", ".$row[$value4].", ".$row[$value5].", ".$row[$value6]."</option>";
	}
	oci_free_statement($query);
}
